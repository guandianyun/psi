 ...
 *
 */
public class FlowIncomeEvent extends BaseTraderEvent {

	private static final Log LOG = Log.getLog(FlowIncomeEvent.class);
	private String action; // 执行的方法名，通过这个参数判断需要执行什么行为
	private List<Object> paramList; // 参数列表，按照接口方法顺序存入

	public FlowIncomeEvent(String action) {
		this.action = action;
	}
	...
	protected void dealBiz() throws Exception {
		FlowIncomeService flowIncomeService = Aop.get(FlowIncomeService.class);
		Ret ret = Ret.ok();
		if(StringUtils.equals(action, "create")) {
			...
			ret = flowIncomeService.create((TraderIncomeExpenses) incomeExpensesObject, (List<TraderIncomeExpensesFund>)orderFundListObject);
			...
			ret = flowIncomeService.update((TraderIncomeExpenses) incomeExpensesObject, (List<TraderIncomeExpensesFund>)orderFundListObject);
			...
			ret = flowIncomeService.disable((List<Integer>) idsObject);
			...
			ret = flowIncomeService.audit((List<Integer>) idsObject, (AuditStatusEnum)auditStatusObject, (String)auditDescObject, (Integer)auditorIdObject);
		}
		responseMsg(ret);
	}
