 ...
 *
 */
public class AccountInfoEvent extends BaseTraderEvent {

	private static final Log LOG = Log.getLog(AccountInfoEvent.class);
	private String action; // 执行的方法名，通过这个参数判断需要执行什么行为
	private List<Object> paramList; // 参数列表，按照接口方法顺序存入

	public AccountInfoEvent(String action) {
		this.action = action;
	}
	...
	protected void dealBiz() throws Exception {
		AccountInfoService accountInfoService = Aop.get(AccountInfoService.class);
		Ret ret = Ret.ok();
		if(StringUtils.equals(action, "create")) {
			Object balanceAccountObject = paramList.get(0);
			if(balanceAccountObject == null) {
				responseMsg(Ret.fail("参数异常"));
				return;
			}
			ret =  accountInfoService.create((TraderBalanceAccount)balanceAccountObject);
			...
			ret =  accountInfoService.update((TraderBalanceAccount)balanceAccountObject);
