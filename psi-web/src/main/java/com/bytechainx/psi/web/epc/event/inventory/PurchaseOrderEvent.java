 ...
 *
 */
public class PurchaseOrderEvent extends BaseTraderEvent {

	private static final Log LOG = Log.getLog(PurchaseOrderEvent.class);
	private String action; // 执行的方法名，通过这个参数判断需要执行什么行为
	private List<Object> paramList; // 参数列表，按照接口方法顺序存入

	public PurchaseOrderEvent(String action) {
		this.action = action;
	}
	...
	protected void dealBiz() throws Exception {
		PurchaseOrderService purchaseOrderService = Aop.get(PurchaseOrderService.class);
		Ret ret = Ret.ok();
		if(StringUtils.equals(action, "create")) {
			Object purchaseOrderObject = paramList.get(0);
			...
			ret = purchaseOrderService.create((PurchaseOrder) purchaseOrderObject, (List<PurchaseOrderGoods>)orderGoodListObject, (List<PurchaseOrderFund>)orderFundListObject, (List<PurchaseOrderFee>)orderFeeListObject, (List<PurchaseOrderCost>)orderCostListObject);
			...
			ret = purchaseOrderService.update((PurchaseOrder) purchaseOrderObject, (List<PurchaseOrderGoods>)orderGoodListObject, (List<PurchaseOrderFund>)orderFundListObject, (List<PurchaseOrderFee>)orderFeeListObject, (List<PurchaseOrderCost>)orderCostListObject);
			...
			ret = purchaseOrderService.disable((List<Integer>) idsObject);
			...
			ret = purchaseOrderService.audit((List<Integer>) idsObject, (AuditStatusEnum)auditStatusObject, (String)auditDescObject, (Integer)auditorIdObject);
		}
		responseMsg(ret);
	}
